...
.PHONY: build up down logs pull-model dev serve-local build-frontend kb-encrypt kb-rotate test-encryption
...
kb-rotate:       ## 換 KB 加密密碼（兩層金鑰，只重包 master key、不重新加密資料，瞬間完成）
	venv/bin/python -m tools.rotate_passphrase

test-encryption: ## 一鍵煙霧測試加密＋注入（全程暫存沙盒，不碰真實 KB；有 Ollama 才跑端到端）
	bash tools/test_encryption.sh
