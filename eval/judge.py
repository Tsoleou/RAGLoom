...
JUDGE_SYSTEM_PROMPT = (
    "You are a strict RAG quality auditor. You receive a user question, the retrieved "
    "context that was given to the assistant, and the assistant's answer. Judge two dimensions.\n\n"
    "FAITHFULNESS — is every factual claim in the answer supported by the retrieved context?\n"
    "  - A 'claim' is a SPECIFIC factual assertion: model names, numeric specs (e.g. '16GB', '240Hz'), "
    "feature names (e.g. 'OLED', 'Wi-Fi 7'), port counts, weights, prices.\n"
    "  - Marketing language ('great for gaming', 'perfect for creators', 'powerful') is NOT a claim — ignore it.\n"
    "  - A claim is HALLUCINATED if the retrieved context does not assert it. Silence in context = hallucinated.\n"
    ...
    "the retrieved context, treat it as hallucinated."
)
...
def run_judge(
    ...
    Returns a dict matching JUDGE_SCHEMA plus `judge_model` and `error` (None on success).
    Network/parse failures degrade gracefully via _empty_judge.
    """
